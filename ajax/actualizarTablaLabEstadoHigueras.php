   ...
   if($_SESSION['tipo_usuario'] == "Administrador"){
       ...
       case '5'://laboratorio -> mostramos solo pacientes de laboratorio
           $query = "SELECT * 
               ...
               INNER JOIN muestras m ON p.id_paciente = m.pacientes_id_paciente
               INNER JOIN trazabilidad t ON m.id_muestra = t.muestras_id_muestra
               WHERE fecha_estado = (SELECT MAX(fecha_estado) FROM trazabilidad WHERE m.id_muestra = muestras_id_muestra) AND estado_trazabilidad = 'APROBADO HPL'
               group by m.id_muestra ORDER BY p.fecha_ingreso DESC";
           break; 
               ...
               switch ($_SESSION['tipo_usuario']) {//tipo de usuario
                   ...
                   //cuando revisamos el tipo de usuario revisamos el area (laboratorio o especialidades) con el motivo de anexar a la tablas las opciones de recibido y aprobado en laborarorio y recibido en administrativo 
                       ...
                       <tr>
                           ...
                           <td><button onclick="iniciarTransitoHigueras('.$row['id_muestra'].')" type="button" class="btn btn-success btn-sm" style="width:30px; height:30px;"><i class="fas fa-check"></i></button>
                           <button onclick="reprobarMuestrasAprobadasLabHpl('.$row['id_muestra'].')" type="button" class="btn btn-danger btn-sm" style="width:30px; height:30px;"><i class="fas fa-times"></i></button>
                   ...
                   case 'Administrativo'://solo para especialidades 
                       ...
                       <tr>
                           ...
                           <td id="botonesTabla">
                               <div class="btn-group" role="group" aria-label="Basic example">
                                 <button onclick="obtenerDatosPacientes('.$row['id_paciente'].')" type="button" class="btn btn-info" data-toggle="modal" data-target="#myModalActualizarPaciente"><span><i class="fas fa-edit"></i></span></button>
                                 <button id="botonImprimir" onclick="printRow('.$row['id_muestras'].')" type="button" class="btn btn-success"><span><i class="fas fa-print"></i></span></button>
                               </div>
                           </td>
                       </tr>';
                       break;           
