   ...
   */
   ...
   if($_SESSION['tipo_usuario'] == "Administrador"){
      ...
      $query = "SELECT * 
               ...
               INNER JOIN trazabilidad t ON m.id_muestra = t.muestras_id_muestra
               WHERE fecha_estado = (SELECT MAX(fecha_estado) FROM trazabilidad WHERE m.id_muestra = muestras_id_muestra 
               ) AND estado_trazabilidad = 'RECIBIDO HIGUERAS'
               group by m.id_muestra ORDER BY p.fecha_ingreso DESC";
   }else{
      //luego de saber si es administrador filtramos la consulta dependiendo del area a la que pertenece
       ...
       case '2'://Endoscopia -> mostramos solo pacientes de Endoscopia
           $query = "SELECT *
                     ...
                     INNER JOIN muestras m ON p.id_paciente = m.pacientes_id_paciente
                     INNER JOIN trazabilidad t ON m.id_muestra = t.muestras_id_muestra
                     WHERE m.id_muestra = t.muestras_id_muestra AND p.areas_id_area = 2
                     GROUP BY m.id_muestra ORDER BY p.fecha_ingreso DESC";
           break;
       ...
       case '3'://Hospitalizados -> mostramos solo pacientes de hospitalizados
           $query = "SELECT *
                     ...
                     INNER JOIN muestras m ON p.id_paciente = m.pacientes_id_paciente
                     INNER JOIN trazabilidad t ON m.id_muestra = t.muestras_id_muestra
                     WHERE m.id_muestra = t.muestras_id_muestra AND p.areas_id_area = 3
                     GROUP BY m.id_muestra ORDER BY p.fecha_ingreso DESC";
           break;
       ...
       case '5'://laboratorio -> mostramos solo pacientes de laboratorio
           $query = "SELECT * 
               ...
               INNER JOIN muestras m ON p.id_paciente = m.pacientes_id_paciente
               INNER JOIN trazabilidad t ON m.id_muestra = t.muestras_id_muestra
               WHERE fecha_estado = (SELECT MAX(fecha_estado) FROM trazabilidad WHERE m.id_muestra = muestras_id_muestra) AND estado_trazabilidad = 'APROBADO'
               group by m.id_muestra ORDER BY p.fecha_ingreso DESC";
           break; 
       ...
       case '6'://Urgencias -> mostramos solo pacientes de Urgencias
           $query = "SELECT *
                     ...
                     INNER JOIN muestras m ON p.id_paciente = m.pacientes_id_paciente
                     INNER JOIN trazabilidad t ON m.id_muestra = t.muestras_id_muestra
                     WHERE m.id_muestra = t.muestras_id_muestra AND p.areas_id_area = 6
                     GROUP BY m.id_muestra ORDER BY p.fecha_ingreso DESC";
           break;
       }
   }
               ...
               switch ($_SESSION['tipo_usuario']) {//tipo de usuario
                   ...
                   case 'Administrador':
                       ...
                       <tr>
                           ...
                           <td>'.$fecha_chilena.'</td>
                           <td>'.$row['estado_trazabilidad'].'</td>
                           
                           <td id="botonesTabla">
                               <div class="btn-group" role="group" aria-label="Basic example">
                                 <button onclick="obtenerDatosPacientes('.$row['id_paciente'].')" type="button" class="btn btn-info" data-toggle="modal" data-target="#myModalActualizarPacienteAdmin" data-backdrop="static" data-keyboard="false"><span><i class="fas fa-edit"></i></span></button>
                                 <button onclick="eliminarPaciente('.$row['id_paciente'].')" type="button" class="btn btn-danger"><span><i class="fas fa-trash-alt"></i></span></button>
                   ...
                   //cuando revisamos el tipo de usuario revisamos el area (laboratorio o especialidades) con el motivo de anexar a la tablas las opciones de recibido y aprobado en laborarorio y recibido en administrativo 
                       ...
                       <tr>
                           ...
                           <td>'.$row['num_frasco'].'</td>
                           <td>'.$row['estado_trazabilidad'].'</td> 
                           <td><button onclick="AprobarMuestra('.$row['id_paciente'].')" type="button" class="btn btn-warning btn-sm">Aprobar muestra</button></td>
                       ...
                       <tr>
                           ...
                           <td id="botonesTabla">
                               <div class="btn-group" role="group" aria-label="Basic example">
                                 <button onclick="obtenerDatosPacientes('.$row['id_paciente'].')" type="button" class="btn btn-info" data-toggle="modal" data-target="#myModalActualizarPaciente" data-backdrop="static" data-keyboard="false"><span><i class="fas fa-edit"></i></span></button>
                                 <button onclick="eliminarPaciente('.$row['id_paciente'].')" type="button" class="btn btn-danger"><span><i class="fas fa-trash-alt"></i></span></button>
                                 <button id="botonImprimir" onclick="printRow('.$row['id_muestra'].')" type="button" class="btn btn-success"><span><i class="fas fa-print"></i></span></button>
                               </div>
                           </td>
                   ...
                   case 'Basico':
                       ...
                       <tr>
                           ...
                           <td>'.$fecha_chilena.'</td>
                           <td>'.$row['estado_trazabilidad'].'</td>
                           <td><button onclick="AprobarMuestra('.$row['id_paciente'].')" type="button" class="btn btn-success btn-sm" style="width:30px; height:30px;"><i class="fas fa-check"></i></button>
                           <button onclick="AprobarMuestra('.$row['id_paciente'].')" type="button" class="btn btn-danger btn-sm" style="width:30px; height:30px;"><i class="fas fa-times"></i></button></td>
                       </tr>';
                       }else{
                           $data .= '
                   ...
                   case 'Administrativo'://solo para especialidades 
                       ...
                       <tr>
                           ...
                           <td>'.$row['areas_id_area'].'</td>
                           <td>'.$fecha_chilena.'</td>                   
                           <td><span class="badge badge-success">'.$row['estado_trazabilidad'].'</span></td>
                           ...
                           <td>
                           <button onclick="recepcionarMuestraAdministrativo('.$row['id_muestra'].')" type="button" class="btn btn-success btn-sm" style="width:30px; height:30px;"><i class="fas fa-check"></i></button>
                           </td>
                       </tr>';
                       break;           
               }    
