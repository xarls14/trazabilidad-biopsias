   ...
   */
   ...
   if($_SESSION['tipo_usuario'] == "Administrador"){
       ...
       case '5'://laboratorio -> mostramos solo pacientes de laboratorio
           $query = "SELECT * 
               ...
               INNER JOIN trazabilidad t ON m.id_muestra = t.muestras_id_muestra
               WHERE fecha_estado = (SELECT MAX(fecha_estado) FROM trazabilidad WHERE m.id_muestra = muestras_id_muestra 
               ) AND estado_trazabilidad = 'RECHAZADO HPL'
               group by m.id_muestra ORDER BY p.fecha_ingreso DESC";
           break; 
   ...
   if(mysqli_num_rows($result) > 0)
               ...
               switch ($_SESSION['tipo_usuario']) {//tipo de usuario
                   ...
                   case 'Administrador':
                       ...
                       <tr>
                           ...
                           <td>'.$fecha_chilena.'</td>
                           <span class="badge badge-danger">'.$row['estado_trazabilidad'].'</span>
                   ...
                   //cuando revisamos el tipo de usuario revisamos el area (laboratorio o especialidades) con el motivo de anexar a la tablas las opciones de recibido y aprobado en laborarorio y recibido en administrativo 
                       ...
                       <tr>
                           ...
                           <td>'.$row['areas_id_area'].'</td>
                           <td>'.$fecha_chilena.'</td>
                           <td><span class="badge badge-danger">'.$row['estado_trazabilidad'].'</span></td>


                           <td id="botonesTabla">
                               <div class="btn-group" role="group" aria-label="Basic example">
                                 <button onclick="updateRechazadoHplToAprobadoHPL('.$row['id_muestra'].')" type="button" class="btn btn-success btn-sm" style="width:30px; height:30px;"><i class="fas fa-check"></i></button>
                               </div>
                           </td>
                           <td>
                              <button onclick="updateRechazadoHplToAprobadoHPL('.$row['id_muestra'].')" type="button" class="btn btn-primary btn-sm" style="width:30px; height:30px;"><i class="fas fa-search"></i></button>
                           </td>
       ...
       else
       {
           ...
           $data .= '<tr><td colspan="8">¡No se encontraron datos!</td></tr>';
       }
