   ...
   if($_SESSION['tipo_usuario'] == "Administrador"){
       ...
       case '5'://laboratorio -> mostramos solo pacientes de laboratorio
           $query = "SELECT * 
               ...
               INNER JOIN trazabilidad t ON m.id_muestra = t.muestras_id_muestra
               WHERE fecha_estado = (SELECT MAX(fecha_estado) FROM trazabilidad WHERE m.id_muestra = muestras_id_muestra 
               ) AND estado_trazabilidad = 'SIN ESTADO'
               group by m.id_muestra ORDER BY p.fecha_ingreso DESC";
           break; 
               ...
               switch ($_SESSION['tipo_usuario']) {//tipo de usuario
                   ...
                   case 'Administrador':
                       ...
                       <tr>
                           ...
                           <td>'.$fecha_chilena.'</td>
                           <td><span class="badge badge-light">'.$row['estado_trazabilidad'].'</span></td>
                   ...
                   //cuando revisamos el tipo de usuario revisamos el area (laboratorio o especialidades) con el motivo de anexar a la tablas las opciones de recibido y aprobado en laborarorio y recibido en administrativo 
                       ...
                       <tr>
                           ...
                           <td>'.$fecha_chilena.'</td>
                           <td><span class="badge badge-light">'.$row['estado_trazabilidad'].'</span></td>
                           <td><button onclick="aprobarMuestrasDesdeUnidades('.$row['id_muestra'].')" type="button" class="btn btn-success btn-sm" style="width:30px; height:30px;"><i class="fas fa-check"></i></button>
                           <button onclick="abrirModalRechazarMuestraLabHpl(id)" type="button" class="btn btn-danger btn-sm" style="width:30px; height:30px;" data-backdrop="static" data-keyboard="false"><i class="fas fa-times"></i></button></td>
                       </tr>';
                       }else{
                           $data .= '
                   ...
                   case 'Basico':
                       ...
                       <tr>
                           ...
                           <td><span class="badge badge-light">'.$row['estado_trazabilidad'].'</span></td>
                           <td><button onclick="aprobarMuestrasDesdeUnidades('.$row['id_muestra'].')" type="button" class="btn btn-success btn-sm" style="width:30px; height:30px;"><i class="fas fa-check"></i></button>
                           <button onclick="reprobarMuestrasDesdeUnidades('.$row['id_muestra'].')" type="button" class="btn btn-danger btn-sm" style="width:30px; height:30px;"><i class="fas fa-times"></i></button></td>
                       </tr>';
                       }else{
                           $data .= '
