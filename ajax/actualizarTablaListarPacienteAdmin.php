<?php
   ...
   */
   ...
   if($_SESSION['tipo_usuario'] == "Administrador"){
     ...
     $query = "SELECT * FROM pacientes ORDER BY id_paciente DESC";
       ...
       case '1'://Pabellon -> mostramos solo pacientes de pabellon
           $query = "SELECT * 
                     FROM pacientes WHERE areas_id_area = 1 ORDER BY id_paciente DESC";
           break;
       
       case '2'://Endoscopia -> mostramos solo pacientes de Endoscopia
           $query = "SELECT * 
                     FROM pacientes WHERE areas_id_area = 2 ORDER BY id_paciente DESC";
           break;
       
       case '3'://Hospitalizados -> mostramos solo pacientes de hospitalizados
           $query = "SELECT * 
                     FROM pacientes WHERE areas_id_area = 3 ORDER BY id_paciente DESC";
           break;
   
       case '4'://Especialidades -> mostramos solo pacientes de Especialidades
           $query = 
               "SELECT * 
               FROM pacientes WHERE areas_id_area = 4 ORDER BY id_paciente DESC";
           break;
       ...
       case '6'://Urgencias -> mostramos solo pacientes de Urgencias
           $query = "SELECT * 
                     FROM pacientes WHERE areas_id_area = 6 ORDER BY id_paciente DESC";
           break;    
       }
   }
               ...
               switch ($_SESSION['tipo_usuario']) {//tipo de usuario
                       ...
                       <tr>
                           ...
                           <td id="botonesTabla">
                               <div class="btn-group" role="group" aria-label="Basic example">
                                 <button onclick="obtenerDatosPacientesAdmin('.$row['id_paciente'].')" type="button" class="btn btn-info" data-toggle="modal" data-target="#myModalActualizarPacienteAdmin" data-backdrop="static" data-keyboard="false"><span><i class="fas fa-edit"></i></span></button>
                               </div>
                           </td>
                       </tr>';
                       break;
                   ...
                   //cuando revisamos el tipo de usuario revisamos el area (laboratorio o especialidades) con el motivo de anexar a la tablas las opciones de recibido y aprobado en laborarorio y recibido en administrativo 
                       ...
                       <tr>
                           ...
                           <td>'.$row['areas_id_area'].'</td>
                           <td>'.$fecha_parseada[0].'</td>
                           <td>'.$row['estado_trazabilidad'].'</td>
                           <td><button onclick="AprobarMuestra('.$row['id_muestra'].')" type="button" class="btn btn-success btn-sm" style="width:30px; height:30px;"><i class="fas fa-check"></i></button>
                           <button onclick="AprobarMuestra('.$row['id_muestra'].')" type="button" class="btn btn-danger btn-sm" style="width:30px; height:30px;"><i class="fas fa-times"></i></button></td>
                       </tr>';
                       }else{
                           $data .= '
                       ...
                       <tr>
                           ...
                           <td id="botonesTabla">
                               <div class="btn-group" role="group" aria-label="Basic example">
                                 <button onclick="obtenerDatosPacientesAdmin('.$row['id_paciente'].')" type="button" class="btn btn-info" data-toggle="modal" data-target="#myModalActualizarPacienteAdmin" data-backdrop="static" data-keyboard="false"><span><i class="fas fa-edit" data-backdrop="static" data-keyboard="false"></i></span></button>
                               </div>
                           </td>
                       </tr>';
                       }
